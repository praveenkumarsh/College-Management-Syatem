...
public class DaoStudent {
    ...
    public static int update(String name, String password) {
        ...
        try {
            Connection con = DB.getConnection();
            PreparedStatement ps = con.prepareStatement("UPDATE student SET name=?,password=? WHERE name=?");
    ...
    public static int update(String name, String password, String email, String address, String city, String contact, String imgpath) {
        ...
        try {
            Connection con = DB.getConnection();
            PreparedStatement ps = con.prepareStatement("UPDATE student SET name=?,password=?,email=?,address=?,city=?,contact=?,profilepic=? WHERE name=?");
            System.out.println("hello");
            ps.setString(1, name);
            ps.setString(2, password);
            ps.setString(3, email);
